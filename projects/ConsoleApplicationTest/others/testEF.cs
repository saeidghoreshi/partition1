    ...
    class testEF
    {
        public void test()
        {
            ...
            using (TransactionScope trans = new TransactionScope())
            {
                ...
                var test1 = new test()
                {
                    name = "New Entry  -  " + DateTime.Today.Ticks.ToString()
                };
                var testReview1 = new testreview()
                {
                    rate = 1000
                };
                test1.testreview.Add(testReview1);
                ctx.test.AddObject(test1);
                ctx.SaveChanges();

                Console.WriteLine("New TEST ID Inserted : " + test1.id);
                Console.WriteLine("New TEST REVIEW ID Inserted : " + testReview1.id);
