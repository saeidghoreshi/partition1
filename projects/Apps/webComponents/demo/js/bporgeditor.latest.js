/**
 * @preserve Basic Primitives orgEditor Demo v1.0.29
 *
...
primitives.orgeditor.Controller.prototype._createLayout = function () {
	...
	if (this.options.editMode) {
		contentString += '<div  style="padding: 0px; margin:10px;"><div style="width:120px;" name="options-button">Options</div><div style="width:120px;" name="json-button">JSON</div></div>';
	}
	...
	if (this.dlgCodeMirror != null) {
		self.dlgCodeMirror.bpDlgCodeMirror("option", {
			...
			"update": function (event, content) {
				...
				self._updateItemConfigs(options.rootItem);
...
primitives.orgeditor.Controller.prototype._updateItemConfigs = function (itemConfig) {
	var index, len;
	...
	for (index = 0, len = itemConfig.items.length; index < len; index+=1) {
		this._updateItemConfigs(itemConfig.items[index]);
	}
};
...
primitives.orgeditor.Controller.prototype._getOrgDiagramConfig = function () {
	...
	this._updateItemConfigs(self.options.rootItem);
...
primitives.orgeditor.DlgItemConfig = function () {
	...
	this.enums = {
		itemType: { name: { Regular: 0, Assistant: 1, Adviser: 2, SubAssistant: 4, SubAdviser: 5 }, title: "Item type", isString: false },
